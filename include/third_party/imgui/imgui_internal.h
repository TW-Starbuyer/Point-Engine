...
struct ImDrawDataBuilder;           // Helper to build a ImDrawData core
...
struct ImGuiTableInstanceData;      // Storage for one core of a same table
...
// You may want to create your own core of you try to ImDrawList completely without ImGui. In that case, watch out for future changes to this structure.
...
enum ImGuiDockNodeFlagsPrivate_
{
    ...
    ImGuiDockNodeFlags_HiddenTabBar             = 1 << 13,  // Saved // Tab bar is hidden, with a triangle in the corner to show it again (NB: actual tab-bar core may be destroyed as this is only used for single-window tab bar)
...
// Every core of ImGuiViewport is in fact a ImGuiViewportP.
...
struct ImGuiTableColumn
{
    ...
    float                   WidthMax;                       // Maximum width (FIXME: overwritten by each core)
...
// Per-core data that needs preserving across frames (seemingly most others do not need to be preserved aside from debug needs. Does that means they could be moved to ImGuiTableTempData?)
// sizeof() ~ 24 bytes
...
struct IMGUI_API ImGuiTable
{
    ...
    ImS16                       InstanceInteracted;         // Mark which core (generally 0) of the same ID is being interacted with
    float                       RowPosY1;
    float                       RowPosY2;
    ...
    bool                        HasScrollbarYCurr;          // Whether ANY core of this table had a vertical scrollbar during the current frame.
    bool                        HasScrollbarYPrev;          // Whether ANY core of this table had a vertical scrollbar during the previous.
...
struct ImGuiTableSettings
{
    ...
    ImGuiTableColumnIdx         ColumnsCountMax;        // Maximum number of columns this settings core can store, we can recycle a settings core with lower number of columns but not higher
    bool                        WantApply;              // Set when loaded from .ini data (to enable merging/loading .ini data into an already running context)
