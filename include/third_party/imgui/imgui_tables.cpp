...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    // - core numbers varying/unstable. may not be a direct problem for users, but could make outside access broken or confusing, e.g. TestEngine.
    // - can't implement support for ImGuiChildFlags_ResizeY as we need to somehow pull the height data from somewhere. this also needs stable core numbers.
    ...
    // Core data (for core 0, TableID == TableInstanceID)
    ImGuiID instance_id;
...
// + 0 (for ImGuiTable core, we are pooling allocations in g.Tables[])
...
void ImGui::TableBeginApplyRequests(ImGuiTable* table)
{
    ...
    // (We process this in the TableBegin() of the first core of each table)
...
void ImGui::TableSetColumnWidth(int column_n, float width)
{
    ...
    const float max_width = ImMax(min_width, column_0->WidthMax); // Don't use TableCalcMaxColumnWidth() here as it would rely on MinX from last core (#7933)
...
// Clear and initialize empty settings core
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    if (!open)
        return;
    if (table->InstanceCurrent > 0)
        Text("** %d instances of same table! Some data below will refer to last core.", table->InstanceCurrent + 1);
    ...
    for (int n = 0; n < table->InstanceCurrent + 1; n++)
    {
        ...
        BulletText("Core %d: HoveredRow: %d, LastOuterHeight: %.2f", n, table_instance->HoveredRowLast, table_instance->LastOuterHeight);
    }
