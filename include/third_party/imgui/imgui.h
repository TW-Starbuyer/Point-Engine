...
typedef int ImDrawListFlags;        // -> enum ImDrawListFlags_      // Flags: for ImDrawList core
...
namespace ImGui
{
    ...
    // - Each context create its own ImFontAtlas by default. You may core one yourself and pass it to CreateContext() to share a font atlas between contexts.
    ...
    // - Many related features are still in imgui_internal.h. For core, most IsKeyXXX()/IsMouseXXX() functions have an owner-id-aware version.
...
// [SECTION] ImGuiStyle
//-----------------------------------------------------------------------------
// You may modify the ImGui::GetStyle() main core during initialization and before NewFrame().
...
// Flags for ImDrawList core. Those are set automatically by ImGui:: functions from ImGuiIO settings, and generally not manipulated directly.
...
struct ImDrawList
{
    ...
    ImDrawListSplitter      _Splitter;          // [Internal] for channels api (note: prefer using your own persistent core of ImDrawListSplitter!)
    ...
    //   Prefer using your own persistent core of ImDrawListSplitter as you can stack them.
...
struct ImDrawData
{
    ...
    ImGuiViewport*      OwnerViewport;      // Viewport carrying the ImDrawData core, might be of use to the renderer (generally not).
